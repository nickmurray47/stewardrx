...
const fhirAuthReducer = (state = defaultFhirAuthState, action) => {
    switch (action.type) {
...
/* SAMPLE OF HOW CONTEXT DISPATCH WORKS */
const signup = dispatch => async ({ email, password }) => {
    // Try to sign up, modify our state and say we are authenticated
    // take JWT and store it on the device.
    // Dispatch an action to put the token in the state object
    try {
        ...
        // If we can't sign up, we dispatch an action to update state with
        // an error message -> show that message
...
const getAccessToken = uri => (dispatch, getState) => {
...
export const { Provider, Context } = createDataContext(
    fhirAuthReducer,
    { signup, clearErrorMessage, getAccessToken, initializeSmart },
    defaultFhirAuthState
);